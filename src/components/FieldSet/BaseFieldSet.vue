<template>
    <fieldset :class="cx('root')" v-bind="ptm('root')" data-pc-name="fieldset">
        <legend :class="cx('legend')" v-bind="ptm('legend')">
            ...
            <a v-if="toggleable" :id="ariaId + '_header'" v-ripple tabindex="0" role="button"
                :aria-controls="ariaId + '_content'" :aria-expanded="!d_collapsed" :aria-label="buttonAriaLabel"
                @click="toggle" @keydown="onKeyDown" v-bind="{ ...toggleButtonProps, ...ptm('toggler') }">
                <slot name="togglericon" :collapsed="d_collapsed">
                    <component :is="d_collapsed ? 'PlusIcon' : 'MinusIcon'" :class="cx('togglericon')"
                        v-bind="ptm('togglericon')" />
                </slot>
                <slot name="legend">
        ...
        <transition name="p-toggleable-content" v-bind="ptm('transition')">
            <div v-show="!d_collapsed" :id="ariaId + '_content'" :class="cx('toggleablecontent')" role="region"
                :aria-labelledby="ariaId + '_header'" v-bind="ptm('toggleablecontent')">
